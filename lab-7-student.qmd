...
#used chatgpt to help debug: https://chatgpt.com/share/67354531-fa60-8003-8a73-1682625f9c3d
```

*Only weight had missing values and there were 1796 observations missing this value.*
...
*In the weight agreement, when calculating the max minus the min, you left length in the min argument instead of changing it to weight like the others*
...
rescale_column <- function(df, var) {
  df <- df |>
    dplyr::mutate(across({{ var }}, rescale_01))
  
  return(df)
}

#used chatgpt to help debug: https://chatgpt.com/share/67358d67-495c-8003-8cd0-dbb5837250da
...
fish |> 
  rescale_column(var = c(length, weight))
